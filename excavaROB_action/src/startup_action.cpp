...
class StartUpAction
{
  ...
  public:
    ...
    pr2_controllers_msgs::JointTrajectoryGoal armExtensionTrajectory()
    {
      ...
      goal.trajectory.points[ind].positions[3] = -0.35;

      goal.trajectory.points[ind].velocities.resize(4);
      for (size_t j = 0; j < 4; ++j) {
        goal.trajectory.points[ind].velocities[j] = 0.0;
      }
      goal.trajectory.points[ind].time_from_start = ros::Duration(1.85);
      ...
      goal.trajectory.points[ind].positions[3] = -0.3873;

      goal.trajectory.points[ind].velocities.resize(4);
      for (size_t j = 0; j < 4; ++j) {
        goal.trajectory.points[ind].velocities[j] = 0.0;
      }
      goal.trajectory.points[ind].time_from_start = ros::Duration(0.55);
